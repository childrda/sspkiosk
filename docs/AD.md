...
If Google succeeds and AD returns `policy_rejected`, the request becomes `partially_completed`. The encrypted password is retained. Automated retry is not offered for that failure; recovery (replace password / kiosk re-selection) is Prompt 2.
