...
- Connection options: `use_tls=true` for LDAPS (port 636). Do not pass removed v3 keys `use_ssl` / StartTLS-as-`use_tls`. StartTLS (`use_starttls`) is prohibited.
- Named connections are registered with `Container::addConnection($connection, 'sspkiosk-ad')`. `Connection` has no `setName()` / `getName()`.
- Password resets set `$user->unicodepwd` (not a public `setPassword()` method).
- The app's own `config('active-directory.use_ssl')` flag remains for validation only and is not passed into LdapRecord.
